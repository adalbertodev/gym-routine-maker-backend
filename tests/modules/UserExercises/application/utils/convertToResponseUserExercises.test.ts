...
describe('exerciseToResponseExercise', () => {
  test('should return a response exercise with the input values', () => {
    ...
    expect(responseExercise.id).toBe(randomUserExercises._id.value);

    for (const exercise of responseExercise.exercises) {
      const expectedUserExercises = randomUserExercises.exercises.find(exercises => exercises._id.value === exercise.id);
